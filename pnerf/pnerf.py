...
def dihedral_to_point(
    ...
    r_cos_theta = torch.tensor(
        bond_lengths * np.cos(np.pi - bond_angles), device=device
    )
...
def point_to_coordinate(points, device, num_fragments=6):
    """
    ...
    init_matrix = torch.from_numpy(init_matrix).to(device)
    init_coords = [
