...
class ExampleModel(openprotein.BaseModel):
    def __init__(self, embedding_size, minibatch_size, use_gpu):
        ...
        self.hidden_to_labels = nn.Linear(
            self.hidden_size * 2, self.mixture_size, bias=True
        )
    ...
    def init_hidden(self, minibatch_size):
        # No. of layers (* 2 since bidirectional), minibatch_size, hidden size
        ...
        self.hidden_layer = (
            autograd.Variable(initial_hidden_state),
    ...
    def _get_network_emissions(self, original_aa_string):
        ...
        emissions_padded, batch_sizes = torch.nn.utils.rnn.pad_packed_sequence(
            torch.nn.utils.rnn.PackedSequence(
                self.hidden_to_labels(data), bi_lstm_batches
            )
        )
        x = emissions_padded.transpose(0, 1).transpose(
            1, 2
        )  # minibatch_size, self.mixture_size, -1
        x = self.bn(x)
        ...
        output_angles = self.softmax_to_angle(p).transpose(
            0, 1
        )  # max size, minibatch size, 3 (angels)
...
class soft_to_angle(nn.Module):
    def __init__(self, mixture_size):
        ...
        omega_components1 = np.random.uniform(
            0, 1, int(mixture_size * 0.1)
        )  # Initialize omega 90/10 pos/neg
        ...
        phi_components = np.genfromtxt(
            "data/mixture_model_pfam_" + str(mixture_size) + ".txt"
        )[:, 1]
        psi_components = np.genfromtxt(
            "data/mixture_model_pfam_" + str(mixture_size) + ".txt"
        )[:, 2]

        self.phi_components = nn.Parameter(
            torch.from_numpy(phi_components).contiguous().view(-1, 1).float()
        )
        self.psi_components = nn.Parameter(
            torch.from_numpy(psi_components).contiguous().view(-1, 1).float()
        )
...
class RrnModel(openprotein.BaseModel):
    def __init__(self, embedding_size, minibatch_size, use_gpu):
        ...
        self.g = nn.Linear(embedding_size + 9 + self.msg_output_size, 9, bias=True)
