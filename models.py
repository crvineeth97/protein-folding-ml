...
class ExampleModel(openprotein.BaseModel):
    def __init__(self, embedding_size, minibatch_size, use_gpu):
        ...
        self.hidden_to_labels = nn.Linear(
            self.hidden_size * 2, self.mixture_size, bias=True)  # * 2 for bidirectional
    ...
    def init_hidden(self, minibatch_size):
        ...
        initial_hidden_state = torch.zeros(
            self.num_lstm_layers * 2, minibatch_size, self.hidden_size)
    ...
    def _get_network_emissions(self, original_aa_string):
        ...
        self.init_hidden(minibatch_size)
        (data, bi_lstm_batches), self.hidden_layer = self.bi_lstm(
        ...
        x = emissions_padded.transpose(0, 1).transpose(
            1, 2)  # minibatch_size, self.mixture_size, -1
        x = self.bn(x)
        x = x.transpose(1, 2)  # (minibatch_size, -1, self.mixture_size)
        p = torch.exp(self.soft(x))
        output_angles = self.softmax_to_angle(p).transpose(
            0, 1)  # max size, minibatch size, 3 (angels)
...
class soft_to_angle(nn.Module):
    def __init__(self, mixture_size):
        ...
        # Omega intializer
        omega_components1 = np.random.uniform(
            0, 1, int(mixture_size * 0.1))  # Initialize omega 90/10 pos/neg
        omega_components2 = np.random.uniform(
            2, math.pi, int(mixture_size * 0.9))
        omega_components = np.concatenate(
            (omega_components1, omega_components2))
        np.random.shuffle(omega_components)

        phi_components = np.genfromtxt(
            "data/mixture_model_pfam_"+str(mixture_size)+".txt")[:, 1]
        psi_components = np.genfromtxt(
            "data/mixture_model_pfam_"+str(mixture_size)+".txt")[:, 2]

        self.phi_components = nn.Parameter(torch.from_numpy(
            phi_components).contiguous().view(-1, 1).float())
        self.psi_components = nn.Parameter(torch.from_numpy(
            psi_components).contiguous().view(-1, 1).float())
        self.omega_components = nn.Parameter(
            torch.from_numpy(omega_components).view(-1, 1).float())
...
class RrnModel(openprotein.BaseModel):
    def __init__(self, embedding_size, minibatch_size, use_gpu):
        ...
        self.f_to_hid = nn.Linear(
            (embedding_size*2 + 9), self.hidden_size, bias=True)
        self.hid_to_pos = nn.Linear(
            self.hidden_size, self.msg_output_size, bias=True)
        # (last state + orginal state)
        self.g = nn.Linear(embedding_size + 9 +
                           self.msg_output_size, 9, bias=True)
    ...
    def _get_network_emissions(self, original_aa_string):
        ...
        for i in range(self.recurrent_steps):
            combined_features = torch.cat(
                (embedding_padded, backbone_atoms_padded), dim=2)
            for idx, aa_features in enumerate(combined_features.transpose(0, 1)):
                # aa_count * output size
                msg = pass_messages(aa_features, self.f, self.use_gpu)
                backbone_atoms_padded[:, idx] = self.g(
                    torch.cat((aa_features, msg), dim=1))

        output, batch_sizes = calculate_dihedral_angles_over_minibatch(
            original_aa_string, backbone_atoms_padded, batch_sizes_backbone, self.use_gpu)
