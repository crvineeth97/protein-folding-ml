...
class H5PytorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        mask = torch.Tensor(self.h5pyfile['mask'][index, :]).type(
            dtype=torch.uint8)
        prim = torch.masked_select(torch.Tensor(
            self.h5pyfile['primary'][index, :]).type(dtype=torch.long), mask)
        tertiary = torch.Tensor(
            self.h5pyfile['tertiary'][index][:int(mask.sum())])  # max length x 9
...
def evaluate_model(data_loader, model):
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        for primary_sequence, tertiary_positions, predicted_pos, predicted_backbone_atoms in minibatch_data:
            ...
            predicted_coords = predicted_backbone_atoms[:len(primary_sequence)].transpose(
                0, 1).contiguous().view(-1, 3).detach()
...
def draw_plot(fig, plt, validation_dataset_size, sample_num, train_loss_values,
    ...
    def draw_with_vars():
        ...
        plt.title("Training progress (" +
                  str(validation_dataset_size) + " samples in validation set)")
        ...
        ax.set_xlabel(
            'Minibatches processed (=network updates)', color='black')
...
def calculate_dihedral_angles_over_minibatch(atomic_coords_padded, batch_sizes, use_gpu):
    ...
    for idx, _ in enumerate(batch_sizes):
        angles.append(calculate_dihedral_angels(
            atomic_coords[idx][:batch_sizes[idx]], use_gpu))
...
def get_structure_from_angles(aa_list_encoded, angles):
    ...
    phi_list = angles[1:, 1]
    psi_list = angles[:-1, 2]
    assert len(aa_list) == len(phi_list) + \
        1 == len(psi_list)+1 == len(omega_list)+1
    structure = PeptideBuilder.make_structure(aa_list,
                                              list(
                                                  map(lambda x: math.degrees(x), phi_list)),
                                              list(
                                                  map(lambda x: math.degrees(x), psi_list)),
...
def calc_pairwise_distances(chain_a, chain_b, use_gpu):
    distance_matrix = torch.Tensor(
        chain_a.size()[0], chain_b.size()[0]).type(torch.float)
    ...
    for i, row in enumerate(chain_a.split(1)):
        distance_matrix[i] = torch.sum(
            (row.expand_as(chain_b) - chain_b) ** 2, 1).view(1, -1)
...
def get_backbone_positions_from_angular_prediction(angular_emissions, batch_sizes, use_gpu):
    ...
    coordinates = pnerf.point_to_coordinate(
        points, use_gpu) / 100  # devide by 100 to angstrom unit
...
def calc_avg_drmsd_over_minibatch(backbone_atoms_padded, actual_coords_padded, batch_sizes):
    ...
    for idx, backbone_atoms in enumerate(backbone_atoms_list):
        actual_coords = actual_coords_list[idx].transpose(
            0, 1).contiguous().view(-1, 3)
        drmsd_avg += calc_drmsd(backbone_atoms.transpose(0,
                                                         1).contiguous().view(-1, 3), actual_coords)
...
def pass_messages(aa_features, message_transformation, use_gpu):
    ...
    eye = torch.eye(aa_count, dtype=torch.uint8).view(-1).expand(2,
                                                                 feature_size, -1).transpose(1, 2).transpose(0, 1)
    eye_inverted = torch.ones(eye.size(), dtype=torch.uint8) - eye
    if use_gpu:
        eye_inverted = eye_inverted.cuda()
    features_repeated = aa_features.repeat(
        (aa_count, 1)).view((aa_count, aa_count, feature_size))
    aa_messages = torch.stack((features_repeated.transpose(
        0, 1), features_repeated)).transpose(0, 1).transpose(1, 2).view(-1, 2, feature_size)
    # (aa_count^2 - aa_count) x 2 x aa_features     (all pairs except for reflexive connections)
    aa_msg_pairs = torch.masked_select(
        aa_messages, eye_inverted).view(-1, 2, feature_size)
    transformed = message_transformation(
        aa_msg_pairs).view(aa_count, aa_count - 1, -1)
