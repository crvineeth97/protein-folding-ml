...
class BaseModel(nn.Module):
    ...
    def embed(self, original_aa_string):
        ...
        embed_tensor = torch.zeros(
            prot_aa_list.size(0), 21, prot_aa_list.size(2)
        )  # 21 classes
        if self.use_gpu:
    ...
    def compute_loss(self, original_aa_string, actual_coords_list):
        ...
        emissions, backbone_atoms_padded, batch_sizes = self._get_network_emissions(
            original_aa_string
        )
